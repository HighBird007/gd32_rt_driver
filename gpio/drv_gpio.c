...
{
    const pin_ *index = pin_get(pin);
    gpio_bit_write(index->gpio_periph, index->pin, value);
}
...
rt_err_t pin_irq_enable(struct rt_device *device, rt_base_t pin, rt_uint32_t enabled){
        ...
        if(enabled == PIN_IRQ_ENABLE){
            ...
            exti_init((exti_line_enum)(index->pin),EXTI_INTERRUPT,irq_mode);
...
void GD32_GPIO_EXTI_IRQHandler(uint8_t index){
    ...
    exti_interrupt_flag_clear(1<<index);
}
